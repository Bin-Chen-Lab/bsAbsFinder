...
```
Just replace the 'liver hepatocellular carcinoma' and 'LIVER' in below code with other cancer and its corresponding healthy tissue.
The input is case sensitive so please add the cancer.type as shown in total_cancer_count and normal.tissue as shown in total_normal_count.

```r
bulk_DE_surface_antigen(cancer.type = 'liver hepatocellular carcinoma', normal.tissue = 'LIVER', octad_counts_data_path = "~/Downloads/octad.counts.and.tpm.h5")
```
